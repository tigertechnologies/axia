...
import { listarLancamentos } from "@/app/actions/financeiro";
...
export default async function FinanceiroPage({ searchParams }: { searchParams: { periodo?: string } }) {
  ...
  const periodo = (searchParams?.periodo ?? "mes") as any;
  const [{ data: profile }, { data: org }, fin] = await Promise.all([
    supabase.from("profiles").select("nome").eq("id", user.id).maybeSingle(),
    supabase.from("organizations").select("plan_id").eq("owner_id", user.id).maybeSingle(),
    listarLancamentos(periodo),
  ]);
  ...
  return (
    <AppShell nome={profile?.nome ?? "Doutor(a)"} planLabel={planLabelFrom(org?.plan_id ?? null)} counts={{ inbox: 0, nomeacoes: 0, prazos: 0, pericias: 0 }} bell={0}>
      <LancamentosClient itens={fin.itens} resumo={fin.resumo} periodo={periodo} />
    </AppShell>
  );
}
