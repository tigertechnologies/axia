...
import { meuPerfil } from "@/app/actions/marketplace";
...
export default async function MarketplacePage() {
  ...
  const [{ data: profile }, { data: org }, perfil] = await Promise.all([
  ...
  return (
    <AppShell nome={profile?.nome ?? "Doutor(a)"} planLabel={planLabelFrom(org?.plan_id ?? null)} counts={{ inbox: 0, nomeacoes: 0, prazos: 0, pericias: 0 }} bell={0}>
      <MarketplaceClient perfilInicial={perfil} nomePadrao={profile?.nome ?? ""} />
    </AppShell>
  );
}
